...
class Life extends React.Component {
    ...
    componentDidMount() {
        ...
        dispatch({ type: 'app/getLifeBlog' });
    }
    componentWillReceiveProps(nextProps) {
        const firstTd = this.refs.firstTd;
        nextProps.lifeBlog && nextProps.lifeBlog.data ? this.setState({
            blog: nextProps.lifeBlog.data
        }, () => {
            nextProps.lifeBlog.data.length > 0 ? this.addBlog(nextProps.lifeBlog.data[0], nextProps.lifeBlog.data) : "";
            this.showBlog(nextProps.lifeBlog.data);
        }) : "";
    ...
    addEach(data) {
        ...
        data.map((data, i, dataAll) => {
            if (i != 0) {
                //加入style="margin-bottom: 0"防止被antd篡改
